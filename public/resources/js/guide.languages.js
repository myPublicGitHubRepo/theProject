...
var guideLanguages = (function () {
    ...
    function _removeLanguage(c) {
        ...
        if(languages.length > 0){
            ...
            if(i > -1){
                ...
                _removeSelectedLanguageFromDOM(c, i);
            }
        }
    }
    /**
     ...
     * @param {string} c language code
     */
    function _addSelectedLanguageToDOM(c) {
     ...
     */
    function _removeSelectedLanguageFromDOM(c, i) {
        $("#div_lang_" + c).remove();
    ...
    function _addSavedLanguages(){
        ...
        if(languages.length > 0){
            ...
            $.each(languages, function(index, value){
                //_addSelectedLanguageToDOM(value.code, value.language);
                _addSelectedLanguageToDOM(value);
            });
        }else{
    ...
    function _submitLanguages(){
        //sort languages by code
        languages.sort(_compareLanguages);
    ...
    function _tryaddLanguage(input){
        var code = $("#languages option[value='" + input + "']").attr('code');
        if(code != undefined){
            _addLanguage(code, input);
        }
     ...
     */
    function _compareLanguages(a, b)
    {
        if (a < b) return -1;
        if (a > b) return 1;
        return 0;
